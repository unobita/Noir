...
async def incoming_compress_message_f(update):
    ...
    try:
        ...
        nam = opm.replace("_", " ")
        nam = opm.replace(".", " ")
        resolution, bitrate = await media_info(saved_file_path)
        episode_info = get_episode_info(nam, resolution)
        joined_string = f"{episode_info} [{resolution}][Sub][@Ongoing_Sensei].mkv"
    ...
    if os.path.exists(saved_file_path):
        ...
        if o is not None:
            ...
            await upload_start.delete()
            await bot.send_message(
                chat_id, f"**Upload Done, Bot is Free Now !!** \n\nProcess Done at `{now}`"
            )
...
async def convert_video(
    video_file, output_directory, total_time, bot, message, chan_msg, episode_info
):
